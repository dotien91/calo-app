  ...
  return await requestMultiple(listPermission)
    .then((statuses: any) => {
      ...
      if (isBlocked) {
        ...
        // 	actions: [{
        // 		text: languages.chatScreen.cancel
        // 	}, {
        // 		text: "Open Settings",
        // 		onPress: async () => {
        ...
        if (permissionRequestResult.length === 0) return RESULTS.GRANTED;
...
export function grantPermission(
  ...
  return async (
    ...
    console.log(permissionForPlatform, "permissionForPlatform");
    const checkPermissionResult: string = await checkPermission(
      permissionForPlatform,
    );
    ...
    if (checkPermissionResult === RESULTS.GRANTED) {
      ...
      if (checkPermissionResult === RESULTS.DENIED) {
        const requestPermissionResult: string = needRequest
          ? await requestPermission(permissionForPlatform)
          : checkPermissionResult;
        console.log(requestPermissionResult, "requestPermissionResult");

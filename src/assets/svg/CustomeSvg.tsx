import React from "react";
import Svg, { Path, Circle, G } from "react-native-svg";

interface IconProps {
  size?: number;
  color?: string;
  style?: any;
}

// 1. ICON CARB - HÌNH NÃO (SVG BẠN GỬI)
export const IconCarb = ({ size = 14, color = '#F4D03F', style }: IconProps) => (
    <Svg 
      width={size}
      height={size}
      viewBox="0 0 24 24" 
      fill="none"
    >
      <Path 
        d="M12 4L10.8522 5.91303C9.94978 7.41704 9.49857 8.16905 9.49857 9C9.49857 9.83095 9.94978 10.583 10.8522 12.087L11.5713 13.2854C11.7627 13.6045 11.8585 13.7641 12 13.7641C12.1415 13.7641 12.2373 13.6045 12.4287 13.2854L13.1478 12.087C14.0502 10.583 14.5014 9.83095 14.5014 9C14.5014 8.16905 14.0502 7.41704 13.1478 5.91303L12 4ZM12 4V2" 
        stroke={color} 
        strokeWidth="2"
      />
      <Path 
        d="M20.1603 12.5981L17.9296 12.5606C16.1759 12.5311 15.2991 12.5163 14.5794 12.9318C13.8598 13.3473 13.4342 14.114 12.5828 15.6475L11.9045 16.8695C11.7239 17.1949 11.6336 17.3575 11.7043 17.4801C11.7751 17.6027 11.9612 17.6058 12.3332 17.6121L13.7307 17.6356C15.4844 17.6651 16.3612 17.6799 17.0809 17.2644C17.8005 16.8489 18.2262 16.0821 19.0775 14.5486L20.1603 12.5981ZM20.1603 12.5981L21.8923 11.5981" 
        stroke={color} 
        strokeWidth="2"
      />
      <Path 
        d="M3.83972 12.5981L6.07035 12.5606C7.82407 12.5311 8.70093 12.5163 9.42056 12.9318C10.1402 13.3473 10.5658 14.114 11.4172 15.6475L12.0955 16.8695C12.2761 17.1949 12.3664 17.3575 12.2957 17.4801C12.2249 17.6027 12.0388 17.6058 11.6668 17.6121L10.2693 17.6356C8.51561 17.6651 7.63876 17.6799 6.91913 17.2644C6.1995 16.8489 5.77385 16.0821 4.92254 14.5486L3.83972 12.5981ZM3.83972 12.5981L2.10767 11.5981" 
        stroke={color} 
        strokeWidth="2"
      />
      <Path 
        d="M12 15V22.5" 
        stroke={color} 
        strokeWidth="2"
      />
    </Svg>
);

// 2. ICON PROTEIN - ĐÙI GÀ (SVG TRƯỚC ĐÓ)
export const IconProtein = ({ size = 14, color = '#E74C3C', style }: IconProps) => (
    <Svg 
      width={size} 
      height={size} 
      viewBox="0 0 72 72" 
      fill="none"
    >
      {/* GROUP 1: MÀU NỀN (COLOR) */}
      <G>
        {/* Phần xương - Giữ màu trắng nổi bật */}
        <Path 
          fill="#FFFFFF" 
          stroke="none" 
          d="M26,27h-7c-7.7334,0-14,6.2666-14,14s6.2666,14,14,14h29c10.4951,0,19-8.5049,19-19s-8.5049-19-19-19 c-5.6934,0-10.79,2.5137-14.2695,6.4805C31.8965,25.6514,29.0645,27,26,27z"
        />
        {/* Phần thịt - Nhận màu từ props (color) */}
        <Path 
          fill={color} 
          stroke="none" 
          d="M29,31c2.5,0,4.5029-1.4141,6.8164-3.7266C39.2695,23.8213,42.9746,21,48,21c8.2842,0,15,6.7158,15,15 s-6.7158,15-15,15H19c-5.5234,0-10-4.4775-10-10s4.4766-10,10-10l0.0313-0.9756l10.0312-0.0312L29,31z"
        />
        {/* Chấm tròn giữa xương */}
        <Circle cx="48" cy="36" r="5" fill="#FFFFFF" stroke="none"/>
      </G>

      {/* GROUP 2: ĐƯỜNG VIỀN (LINES) */}
      {/* Đổi stroke từ #000000 sang {color} để nó hiển thị rõ trên nền tối */}
      <G>
        <Path 
          fill="none" 
          stroke={color} 
          strokeLinecap="round" 
          strokeLinejoin="round" 
          strokeMiterlimit="10" 
          strokeWidth="2" 
          d="M29,31c2.5,0,4.5029-1.4141,6.8164-3.7266C39.2695,23.8213,42.9746,21,48,21c8.2842,0,15,6.7158,15,15s-6.7158,15-15,15H19 c-5.5234,0-10-4.4775-10-10s4.4766-10,10-10"
        />
        <Path 
          fill="none" 
          stroke={color} 
          strokeLinecap="round" 
          strokeLinejoin="round" 
          strokeMiterlimit="10" 
          strokeWidth="2" 
          d="M26,27h-7c-7.7334,0-14,6.2666-14,14s6.2666,14,14,14h29c10.4951,0,19-8.5049,19-19s-8.5049-19-19-19 c-5.6934,0-10.79,2.5137-14.2695,6.4805C31.8965,25.6514,29.0645,27,26,27z"
        />
        <Circle 
          cx="48" 
          cy="36" 
          r="5" 
          fill="none" 
          stroke={color} 
          strokeLinecap="round" 
          strokeLinejoin="round" 
          strokeMiterlimit="10" 
          strokeWidth="2"
        />
      </G>
    </Svg>
);

// 3. ICON FAT - GIỌT DẦU (Giữ nguyên mẫu trước)
export const IconFat = ({ size = 14, color = '#2ECC71', style }: IconProps) => (
  <Svg width={size} height={size} viewBox="0 0 24 24" fill={color} style={style}>
    <Path d="M12 2C12 2 5 10 5 15C5 19 8 22 12 22C16 22 19 19 19 15C19 10 12 2 12 2Z" />
  </Svg>
);